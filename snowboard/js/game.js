window.addEventListener("DOMContentLoaded", function () {
  ...
  var createScene1 = function () {
    ...
    var camera = new BABYLON.UniversalCamera(
      "Camera",
      new BABYLON.Vector3(-8.273084616, 31.7, -10.0018),
      scene
    );
    camera.minZ = 0.1;
    ...
    var light_spot_r = new BABYLON.SpotLight(
      "spotLightR",
      new BABYLON.Vector3(4, 25, 18),
      new BABYLON.Vector3(0, -1, -1),
      Math.PI / 2,
      2,
      scene
    );
    var light_spot_l = new BABYLON.SpotLight(
      "spotLightL",
      new BABYLON.Vector3(25, 17, 10),
      new BABYLON.Vector3(-4, -1, -1),
      Math.PI / 2,
      2,
      scene
    );
    var light_spot_r2 = new BABYLON.SpotLight(
      "spotLightL",
      new BABYLON.Vector3(18, 20, 5),
    ...
    var light_hemi = new BABYLON.HemisphericLight(
      "hemiLight",
    ...
    let box = BABYLON.MeshBuilder.CreateBox(
      "Box",
      { height: 0.42, width: 0.2, depth: 0.54 },
      scene
    );
    box.position = new BABYLON.Vector3(-9.5858738, 30, -8.74);
    box.rotation.x = 1.75;
    box.rotation.y = 0.67;
    box.rotation.z = 0;
    box.visibility = 0;
    ...
    var mountainMeshTask = assetsManager.addMeshTask(
      "",
      "",
      "models/mountain_merged_scene_3.babylon"
    );
    var snowboardMeshTask = assetsManager.addMeshTask(
      "",
      "",
      "models/snowboarder.babylon"
    );

    snowboardMeshTask.onSuccess = (task) => {
      let i;
      for (i = 0; i < task.loadedMeshes.length; i++) {
        task.loadedMeshes[i].position = new BABYLON.Vector3(
          -9.758738,
          29.65,
          -8.74
        );
    ...
    mountainMeshTask.onSuccess = (task) => {
      ...
      terrain.position = new BABYLON.Vector3(
        -2.875111 - 3.2613009 * t1,
        28.6604 + 2.2135299999999987 * t1,
        1.3026 - 4.8864 * t1
      );
      ...
      let endBox = BABYLON.MeshBuilder.CreateBox(
        "Box",
        { height: 0.42, width: 1, depth: 0.84 },
        scene
      );
      endBox.position = new BABYLON.Vector3(
        20.7982727,
        18.342038725,
        36.211586642
      );
      endBox.rotation.x = 1.75;
      endBox.rotation.y = 0.67;
      endBox.rotation.z = 0;
      endBox.visibility = 0;
      ...
      function startRun() {
        //Terrain
        let anim_terrain = new BABYLON.Animation(
          "terrain_anim",
          "position",
          60,
        ...
        anim_terrain_keys.push({
          frame: 280,
          value: new BABYLON.Vector3(
            -7.5875111 - 3.2613009 * t,
            27.75,
            -5.3026 - 4.8864 * t
          ),
        });
        ...
        mountainAnimatable = scene.beginDirectAnimation(
          terrain,
          [anim_terrain],
          0,
          280,
        ...
        let snowboarderIdleAnimatable = skeleton.beginAnimation(
          "idle",
          true,
          2
        );
      ...
      function cameraJump() {
        ...
        let cameraJumpAnimatable = scene.beginDirectAnimation(
          cam,
          [a],
          0,
          23,
        ...
        let skeleton = scene.getSkeletonByName("Armature");
        let snowboarderJumpAnimatable = skeleton.beginAnimation(
          "jump",
          false,
          1.8,
          () => {
            snowboarderIdleAnimatable = skeleton.beginAnimation(
              "idle",
              true,
              2
            );
      ...
      scene.registerBeforeRender(() => {
        ...
        for (j = 0; j < obstacle.length; j++) {
          if (obstacle[j].intersectsMesh(box, true)) {
            ...
            //console.log('hit');
            if (hits == 0) {
              let snowboarderFallAnimatable = skeleton.beginAnimation(
                "fall",
                false,
                0.9,
                () => {
        ...
        if (box.intersectsMesh(endBox, true) && endHits == 0) {
          ...
          let animCameraEnd = new BABYLON.Animation(
          ...
          let bezierEase2 = new BABYLON.BezierCurveEase(0.22, 1, 0.84, 1);
          animCameraEnd.setKeys(keysCameraEnd);
          animCameraEnd.setEasingFunction(bezierEase2);

          //Camera target animation
          let animCameraTargetEnd = new BABYLON.Animation(
            "cameralandingTargetAnimation",
            "lockedTarget",
            60,
            BABYLON.Animation.ANIMATIONTYPE_VECTOR3,
            BABYLON.Animation.ANIMATIONLOOPMODE_CONSTANT
          ...
          keysCameraTargetEnd.push({
            frame: 100,
            value: new BABYLON.Vector3(-9.463078468, 29.9109974, -8.26246478),
          });
          ...
          scene.beginDirectAnimation(
            camera,
            [animCameraEnd, animCameraTargetEnd],
            0,
            100,
            false
          );
          window.setTimeout(() => {
            let skeleton = scene.getSkeletonByName("Armature");
            snowboarderEndAnimatable = skeleton.beginAnimation(
              "end",
              false,
              0.8
            );
    ...
    function startScene() {
      ...
      let animCamera = new BABYLON.Animation(
        "cameraPositionAnimation",
        "position",
        60,
        BABYLON.Animation.ANIMATIONTYPE_VECTOR3,
      ...
      //Camera target animation
      let animCameraTarget = new BABYLON.Animation(
        "cameralandingTargetAnimation",
        "lockedTarget",
        60,
        BABYLON.Animation.ANIMATIONTYPE_VECTOR3,
